...
const socials = [
  {
    label: 'فيسبوك',
    href: 'https://www.facebook.com/share/1J1EFdNZ4s/',
  ...
  {
    label: 'انستقرام',
    href: 'https://www.instagram.com/amine_jurisport?igsh=MTMzbjZkeGN6dDUwdQ==',
  ...
  {
    label: 'يوتيوب',
    href: 'https://www.youtube.com/@aminefit',
            ...
            {info.map(({ icon: Icon, label, value, href, ltr }) => (
              <a key={label} href={href}
                ...
                <div>
                  ...
                  <p className="text-white font-semibold text-sm" dir={ltr ? 'ltr' : undefined}>{value}</p>
                </div>
              </a>
            ))}
              ...
              <div className="flex gap-3">
                {socials.map(({ label, href, color, path }) => (
                  <a
                    ...
                    title={label}
                    className={`w-11 h-11 flex items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/40 transition-all ${color}`}
                  >

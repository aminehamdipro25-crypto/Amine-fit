...
// Meal macros are computed by summing per-food USDA coefficients across all items.
// Final kcal uses Atwater: (carbs × 4) + (protein × 4) + (fat × 9)
export function generateMenu(ex, mealCount = 5, preferred = '', avoided = '', dayOffset = 0) {
  ...
  return tpls.map(tpl => {
    ...
    // Aggregate macros across all items — Atwater at the end
    let totalCarbs = 0, totalProtein = 0, totalFat = 0

    function pushItem(group, icon, servings, food) {
    ...
    if (sC > 0 && pool.starch.length) {
      ...
      pushItem(grpEx.nameAr, grpEx.icon, sC, food)
      rot.starch++
    }
    ...
    if (mC > 0 && pool.meat.length) {
      ...
      pushItem(EX.meat.nameAr, EX.meat.icon, mC, sorted[rot.meat % sorted.length])
      rot.meat++
    }

    // ── DAIRY ────────────────────────────────────────────────────────────────
    if (dC > 0 && pool.milk.length) {
      pushItem(EX.milk.nameAr, EX.milk.icon, dC, pool.milk[rot.milk % pool.milk.length])
      rot.milk++
    }

    // ── FRUIT ────────────────────────────────────────────────────────────────
    if (fC > 0 && pool.fruit.length) {
      pushItem(EX.fruit.nameAr, EX.fruit.icon, fC, pool.fruit[rot.fruit % pool.fruit.length])
      rot.fruit++
    }
    ...
    if (fR > 0 && pool.fat.length) {
      if (['B','S'].includes(mealCode)) {
        ...
        for (const fat of bFats) {
          ...
          pushItem(EX.fat.nameAr, EX.fat.icon, srv, fat)
          remaining -= srv
          placed++
        }
      } else {
        ...
        pushItem(EX.fat.nameAr, EX.fat.icon, fR, sorted[rot.fat % sorted.length])
        rot.fat++
      }
    }
    ...
    if (vC > 0 && pool.vegetable.length) {
      ...
      pushItem(EX.vegetable.nameAr, EX.vegetable.icon, half, v1)
      if (vC > 1 && pool.vegetable.length > 1) {
        const v2 = pool.vegetable[(rot.vegetable + 1) % pool.vegetable.length]
        pushItem(EX.vegetable.nameAr, EX.vegetable.icon, vC - half, v2)
      }
      rot.vegetable++
    }

    // Atwater: kcal = (C × 4) + (P × 4) + (F × 9)
    const mealKcal = Math.round((totalCarbs * 4) + (totalProtein * 4) + (totalFat * 9))

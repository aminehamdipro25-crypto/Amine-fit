...
// Remove foods that match avoided keywords
...
// Sort food list to prioritize preferred keywords (after filtering avoided)
...
export function calcExchanges(target, goalKey) {
  const goal   = GOALS.find(g => g.key === goalKey) || GOALS[1]
  ...
  // Carb calories split: 55% starches, 20% fruits, 10% vegetables, 15% dairy
  const starches   = Math.max(3, Math.round(carbCal * 0.55 / EX.starch.kcal))
  const fruits     = Math.max(2, Math.round(carbCal * 0.20 / EX.fruit.kcal))
  const vegetables = Math.max(2, Math.round(carbCal * 0.10 / EX.vegetable.kcal))
  const dairy      = Math.max(1, Math.round(carbCal * 0.15 / EX.milk.kcal))

  // Protein from low-fat meat servings
...
export function generateMenu(ex, mealCount = 5, preferred = '', avoided = '') {
  ...
  // Filter avoided foods first, then sort by preference; keep at least 1 item per group
  const filter = (list) => {
  ...
  const sorted = {
    starch:    sortByPreference(filter(FOODS.starch),    preferred),
    meat:      sortByPreference(filter(FOODS.meat),      preferred),
    milk:      filter(FOODS.milk),
    fruit:     filter(FOODS.fruit),
    vegetable: filter(FOODS.vegetable),
    fat:       filter(FOODS.fat),
  }

...
export const FOODS = {
  ...
  meat: [
    ...
    // Medium-fat (~75 kcal / serving): eggs, cheese — ADA medium-fat meat exchange
    { nameAr: 'بيضة مسلوقة',       grams: 50,  suffix: 'غ (بيضة كاملة)',      state: 'مسلوقة',
      keywords: ['egg','بيض','بيضة'],          meal: 'BSD',
      carbsPer100g: 1.1,  proteinPer100g: 13,   fatPer100g: 10,  kcalPerServing: 75 },
    { nameAr: 'جبن مطبوخة',        grams: 30,  suffix: 'غ (قطعة)',            state: 'معالَج',
      keywords: ['جبن','cheese','fromage','جبن مطبوخ'], meal: 'BSD',
      carbsPer100g: 2.2,  proteinPer100g: 17,   fatPer100g: 22,  kcalPerServing: 75 },
  ],
...
export function calcExchanges(target, goalKey, avoided = '', opts = {}) {
  ...
  if (actualKcal > target + 40) {
    const reduce = Math.round((actualKcal - target) / EX.starch.kcal)
    starches = Math.max(3, starches - reduce)
  } else if (actualKcal < target - 40) {
    const add = Math.round((target - actualKcal) / EX.starch.kcal)
...
// ── Protein type → food keyword filter ──────────────────────────────────────
// null = no filter (all meat foods), [] = no animal protein (plant-based days)
export const PROTEIN_FILTERS = {
  mixed:      null,
  chicken:    ['دجاج', 'chicken'],
  fish:       ['سمك', 'fish', 'salmon', 'سلمون', 'سردين', 'sardine', 'جمبري', 'shrimp'],
  red_meat:   ['لحم', 'beef'],
  liver:      ['كبدة', 'liver'],
  tuna:       ['تونة', 'tuna'],
  eggs:       ['بيض', 'بيضة', 'egg'],
  plant:      [],                              // no meat — legumes supply protein
  eggs_plant: ['بيض', 'بيضة', 'egg'],         // eggs only; legumes added via starch
  dairy:      ['جبن', 'cheese', 'fromage'],   // cheese as protein
}

// Starch foods appropriate for each region (used to sort pool — preferred first)
const REGION_STARCH_PREF = {
  maghreb:    ['كسكس', 'خبز عربي', 'فول'],
  gulf:       ['أرز بسمتي', 'أرز', 'تمر', 'خبز عربي'],
  levant:     ['خبز عربي', 'فول', 'حمص'],
  egypt:      ['فول', 'أرز', 'خبز'],
  arab_other: ['أرز', 'خبز عربي', 'تمر'],
}

// ── Seeded offset generator — gives each group a distinct starting position ───
// dayOffset  distinguishes days within a week/month plan
// genSeed    changes every time localPlan() is called (Math.random-based)
// groupSalt  ensures starch / meat / fruit / ... never cycle in lockstep
function seededOffset(dayOffset, genSeed, groupSalt) {
  const base = (Math.abs(dayOffset * 7919 + genSeed * 6271 + groupSalt) | 0) >>> 0
  let s = base
  s = ((s * 1664525) + 1013904223) >>> 0
  return s % 997  // prime modulus larger than any food pool — safe for % pool.length later
}

// ── Smart Menu Generator ─────────────────────────────────────────────────────
// Meal macros use ADA exchange reference values (not USDA per-food) so meal kcal sum = actualKcal.
// proteinKey: optional key from PROTEIN_FILTERS to restrict the meat food pool for this day
// country:    ISO-2 country code — used to prioritize regional staple foods
// genSeed:    per-generation random seed (pass Math.floor(Math.random()*1e6) from the caller)
//             ensures consecutive generations of the same client data produce fresh menus
export function generateMenu(ex, mealCount = 5, preferred = '', avoided = '', dayOffset = 0, proteinKey = 'mixed', country = '', genSeed = 0) {
  ...
  // Apply regional starch prioritization: move local staples to front of pool
  const region = getRegion(country)
  const regionPref = REGION_STARCH_PREF[region] || []
  if (regionPref.length > 0 && pool.starch?.length > 0) {
    pool.starch = [
      ...pool.starch.filter(f => regionPref.some(k => f.nameAr.includes(k) || f.keywords.some(kw => kw.includes(k)))),
      ...pool.starch.filter(f => !regionPref.some(k => f.nameAr.includes(k) || f.keywords.some(kw => kw.includes(k)))),
    ]
  }

  // Apply protein type filter: restrict meat pool to chosen protein source
  const pf = PROTEIN_FILTERS[proteinKey] ?? null
  if (pf !== null) {
    if (pf.length === 0) {
      pool.meat = []  // plant-based day: no animal protein items
    } else {
      const proteinFiltered = pool.meat.filter(f =>
        f.keywords.some(k => pf.some(pk => k.includes(pk) || pk.includes(k)))
      )
      if (proteinFiltered.length > 0) pool.meat = proteinFiltered
      // if no match (e.g. food excluded by avoided), keep original pool as fallback
    }
  }

  // Each group gets its own independent starting index so they never cycle in lockstep.
  // Using different prime salts per group ensures starch[0] ≠ meat[0] ≠ fruit[0] within the same day,
  // and genSeed ensures consecutive generations of the same data produce different selections.
  const rot = {
    starch:    seededOffset(dayOffset, genSeed, 2),
    meat:      seededOffset(dayOffset, genSeed, 3),
    milk:      seededOffset(dayOffset, genSeed, 5),
    fruit:     seededOffset(dayOffset, genSeed, 7),
    vegetable: seededOffset(dayOffset, genSeed, 11),
    fat:       seededOffset(dayOffset, genSeed, 13),
  }
  ...
  return tpls.map(tpl => {
    ...
    function pushItem(group, icon, servings, food, exRef) {
      ...
      // Always use ADA exchange reference kcal — keeps meal totals identical to exchange table
      ...
      items.push({
        group, icon, servings,
        food:   food.nameAr,
        amount: `${g} ${food.suffix}`,
        kcal:   Math.round(exRef.kcal * servings),
      })
    }
    ...
    if (sC > 0 && pool.starch.length) {
      ...
      // Display legume group label/icon when applicable, but always use EX.starch for kcal
      // so meal totals match the exchange table (which counts all starches at EX.starch.kcal)
      const dispEx = food.isLegume ? EX.legume : EX.starch
      pushItem(dispEx.nameAr, dispEx.icon, sC, food, EX.starch)
      rot.starch++
    }

    // ── MEAT / PROTEIN ───────────────────────────────────────────────────────
    if (mC > 0 && pool.meat.length) {
      const isBreakfastOrSnack = ['B', 'S'].includes(mealCode)
      const ctxPool = isBreakfastOrSnack
        ? pool.meat.filter(f => (f.meal || '').includes('B'))
        : pool.meat.filter(f => (f.meal || '').includes(mealCode) || (f.meal || '').includes('L'))
      // Breakfast: never fallback to lunch/dinner meats — skip if no B-appropriate protein found
      const meatPool = (!isBreakfastOrSnack && ctxPool.length === 0) ? pool.meat : ctxPool
...
export function getRegion(country) {
  ...
  if (['TN','MA','DZ','LY','MR'].includes(c))              return 'maghreb'
  if (['QA','AE','SA','KW','BH','OM'].includes(c))          return 'gulf'
  if (['SY','LB','JO','PS'].includes(c))                    return 'levant'
  if (c === 'EG')                                           return 'egypt'
  if (['IQ','YE','SD'].includes(c))                         return 'arab_other'
  // الاتحاد الأوروبي — حوض المتوسط
  if (['IT','ES','PT','GR','MT','CY','FR','HR','SI'].includes(c)) return 'eu_med'
  // الاتحاد الأوروبي — شمال وغرب أوروبا
  if (['DE','NL','BE','SE','DK','FI','IE','AT','LU',
       'GB','CH','NO'].includes(c))                         return 'eu_north'
  // الاتحاد الأوروبي — وسط وشرق أوروبا
  if (['PL','CZ','RO','HU','SK','BG','LT','LV','EE'].includes(c)) return 'eu_east'
  return 'global'
}
...
export const REGION_FOOD_HINTS = {
  ...
  // أوروبا — المتوسط: مطبخ متوسطي غني بالبروتين والخضار
  eu_med:     'pasta, risotto, grilled fish, olive oil, tomatoes, legumes, mozzarella, Greek yogurt, sourdough bread, vegetables',
  // أوروبا — الشمال والغرب: مطبخ متوازن متنوع
  eu_north:   'oats, rye bread, salmon, eggs, dairy, potatoes, chicken, broccoli, berries, cottage cheese',
  // أوروبا — الوسط والشرق: مطبخ غني بالكربوهيدرات والبروتين
  eu_east:    'rye bread, buckwheat, pork lean cuts, eggs, dairy, potatoes, beets, cabbage, sunflower oil',
  global:     '',
}

export const REGION_LABELS = {
  maghreb:    'المغرب العربي (تونس / المغرب / الجزائر / ليبيا / موريتانيا)',
  ...
  eu_med:     'أوروبا المتوسطية (إيطاليا / إسبانيا / اليونان / فرنسا / البرتغال...)',
  eu_north:   'شمال وغرب أوروبا (ألمانيا / هولندا / السويد / المملكة المتحدة...)',
  eu_east:    'وسط وشرق أوروبا (بولندا / التشيك / رومانيا / المجر...)',
  global:     '',
}

// ─── Nutrition Engine v4 — ADA Exchange System 2019 / Mifflin-St Jeor ────────
...
// ── v4: FFM-Based Protein Target ────────────────────────────────────────────
// Protein factors read from GOALS.proteinFactor (loss=2.0, maintain=1.8, gain=2.2 g/kg FFM)
// Anti-sarcopenia: add 0.2 for age > 55. Safety cap: 3.0 g/kg total body weight.
export function calcProteinTarget(weight, bodyFatPct, goalKey, age = 30) {
  ...
  const proteinG = factor * ffm
  const cap = 3.0 * ffm
  return Math.min(proteinG, cap)
}
...
// ── v4: Fiber Estimate from Exchange Counts ──────────────────────────────────
// Starches ≈ 1.5g avg (whole grain ~2g, refined ~1g — realistic mix), vegetables ≈ 2.5g, fruits ≈ 2g
...
// opts = { age=30, bodyFatPct=null, weight=null }
export function calcExchanges(target, goalKey, avoided = '', opts = {}) {
  const { age = 30, bodyFatPct = null, weight = null } = opts
  const goal = GOALS.find(g => g.key === goalKey) || GOALS[1]

  // ── Age-adjusted macro ratios ─────────────────────────────────────────────
  let cR = goal.macros.carbs
  let pR = goal.macros.protein
  let fR = goal.macros.fat

  if (+age > 55) {
    // Shift up to 0.08 from carbs to protein; keep carbs ≥ 0.30
    const shift = Math.min(0.08, cR - 0.30)
    cR -= shift
    pR += shift
  } else if (+age > 35) {
    // 36–55: shift up to 0.04 from carbs to protein
    const shift = Math.min(0.04, cR - 0.30)
    cR -= shift
    pR += shift
  }

  let carbCal = target * cR
  let protCal = target * pR
  let fatCal  = target * fR

  // ── FFM-based protein override ────────────────────────────────────────────
  if (bodyFatPct != null && weight != null) {
    ...
    if (newProtCal !== protCal) {
      ...
      protCal = newProtCal
      // Enforce non-negative floors (three-pass: avoids carbCal going negative after second pass)
  ...
  // Enforce ADA minimum 130g carbs (520 kcal) — prevents sub-optimal intake for elderly/low-calorie plans
  const carbMin = 520
  if (carbCal < carbMin && target >= carbMin) {
    const deficit = carbMin - carbCal
  ...
  // v4 fix: cap starchPct so it never absorbs all of carbCal
  ...
  // Recompute after starch adjustment
  actualKcal =
    starches * EX.starch.kcal + meats * EX.meat.kcal + dairy * EX.milk.kcal +
    fats * EX.fat.kcal + fruits * EX.fruit.kcal + vegetables * EX.vegetable.kcal
  ...
  return {
    ...
    proteinTarget: bodyFatPct != null ? Math.round(calcProteinTarget(weight, bodyFatPct, goalKey, age)) : null,
  }
}
...
export function generateMenu(ex, mealCount = 5, preferred = '', avoided = '', dayOffset = 0) {
  ...
  return tpls.map(tpl => {
    ...
    if (mC > 0 && pool.meat.length) {
      // Strict meal-context filtering: breakfast/snack → eggs & cheese only, never grilled meat/fish
      const contextPool = ['B', 'S'].includes(mealCode)
        ? pool.meat.filter(f => (f.meal || '').includes('B'))
        : pool.meat.filter(f => (f.meal || '').includes(mealCode) || (f.meal || '').includes('L'))
      const meatPool = contextPool.length > 0 ? contextPool : pool.meat

      // Sort preferred to top within context pool
      const sortedMeat = [...meatPool].sort((a, b) =>

...
function sortByPreference(list, preferred = '') {
  ...
  const kws = preferred.toLowerCase().split(/[\s,،]+/)
  return [...list].sort((a, b) => {
    const aMatch = a.keywords.some(k => kws.some(kw => k.includes(kw) || kw.includes(k))) ? -1 : 0
    const bMatch = b.keywords.some(k => kws.some(kw => k.includes(k) || kw.includes(k))) ? -1 : 0
...
export function generateMenu(ex, mealCount = 5, preferred = '') {
  const count  = [3, 4, 5].includes(mealCount) ? mealCount : 5
  const tpls   = TEMPLATES[count]
  const sorted = {
    starch:    sortByPreference(FOODS.starch,    preferred),
    meat:      sortByPreference(FOODS.meat,      preferred),
    milk:      FOODS.milk,
    fruit:     FOODS.fruit,
